...
def test_invalidation_matches_artifact_scope():
    ...
    assert "summarizing" in stage_keys_invalidated_by("writing_outputs")
    assert "summarizing" in stage_keys_invalidated_by("transcribing")
    assert "summarizing" in stage_keys_invalidated_by("summarizing")
